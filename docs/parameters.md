...
STARSolo command line arguments

| Parameter | Description | Type | Default | Required | Hidden |
|-----------|-----------|-----------|-----------|-----------|-----------|
| `starsolo_common_args` | Work for both p3 and p3p5. | `string` | --clip3pAdapterSeq AAAAAAAAAAAA --outSAMtype BAM SortedByCoordinate --outFilterMatchNmin 50 --soloCellReadStats Standard --soloCellFilter CellRanger2.2 10 0.8 10 |  |  |
| `starsolo_p3_args` | Work for only p3 analysis. Must contain '--soloFeatures GeneFull_Ex50pAS'. | `string` | --soloFeatures GeneFull_Ex50pAS --outSAMattributes NH HI nM AS CR UR CB UB GX GN |  |  |
| `starsolo_p3p5_args` | Work for p3+p5 analysis. As p5 UMI is too short to be used by now, no UMI deduplication is performed and count all reads | `string` | --soloFeatures Gene SJ Velocyto --soloUMIdedup NoDedup |  |  |
